...
class TrajectoryBatch(
    ...
    def __new__(cls, env_spec, observations, last_observations, actions,
        ...
        if step_types.dtype != StepType:
            raise ValueError(
                'terminals tensor must be dtype `StepType`, but got tensor '
...
class TimeStep(
    ...
    def terminal(self):
        """Checks if the `TimeStep` is a TERMINAL `TimeStep`.

        Returns:
            bool: True if it is a TERMINAL `TimeStep`.
        """
        return self.step_type is StepType.TERMINAL

    def timeout(self):
        """Checks if the `TimeStep` is a TIMEOUT `TimeStep`.
    ...
    def last(self):
        """Cehck fi the `TimeStep` is the last one in a sequence.

        Returns:
            bool: True if it is a TERMINAL or TIMEOUT `TimeStep`.
        """
